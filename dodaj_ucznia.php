...
<head>
    ...
    <title>E-dziennik - Dodaj ucznia</title>
</head>
<body>
    <header class="Banner">
        <h1>Dziennik</h1>
    ...
    <aside class="Main">
        <form method="post">
            <h2>Dodaj ucznia do wybranej klasy</h2>
            Podaj imie ucznia<input type="text" name="imie"><br>
            Podaj nazwisko ucznia<input type="text" name="nazwisko"><br>
            <?php
                 $conn = mysqli_connect('localhost','root','','e-dziennik2tp1');
        ...
        <?php
            if(isset($_POST['klasa']) || isset($_POST['imie'])) {
                if ($_POST['klasa'] != "" && $_POST['nazwisko'] != "") {
                    ...
                    $q = "INSERT INTO `uczen`(`id`, `imie`, `nazwisko`, `id_klas`) VALUES ('','$imie','$nazwisko',$IDD)";
                    $result = mysqli_query($conn,$q);

                    include "register.php";
                    if ($row) {
                        echo "Uczeń poprawnie dodany do klasy";
                    }   
                }
        }
        ?>
